...
exports.register = async (req, res, next) => {
  try {
    ...
    // DEVELOPMENT HELPER: Print the URL to the console so you don't need a real mailing service to test
    console.log('\\n\\n=============================================');
    ...
    try {
      ...
      res.status(201).json({ success: true, message: 'Registration successful! Please check the terminal output for your verification link.' });
    }
  } catch (err) { next(err); }
};
...
exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: 'Email has been verified successfully. You can now log in.' });
  } catch (err) { next(err); }
};
